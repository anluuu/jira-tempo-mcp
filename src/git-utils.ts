 ...
 */
export function getIssueKeyFromBranch(cwd?: string): string | null {
  try {
 ...
 */
export function getCommitsOnBranch(
  ...
  try {
    ...
    const log = run(
      `git log ${baseBranch}..HEAD --format="%H||%s||%ai||%an" --no-merges`,
      cwd
    );
    if (!log) return [];
 ...
 */
export function getRepoRemoteUrl(cwd?: string): string | null {
  try {
