...
import { getIssueKeyFromBranch, getCurrentBranch, getCommitsOnBranch, getPrUrl, createBranch, getDiffSummary, } from "./git-utils.js";
...
server.registerTool("jira_get_issue", {
    ...
    description: "Fetch details of a JIRA issue (summary, status, assignee, type, priority, description). " +
...
server.registerTool("jira_transition_issue", {
    ...
    description: "Move a JIRA issue to a new status. Use jira_get_transitions first to get valid transition IDs.",
    inputSchema: {
        issueKey: z.string().describe("JIRA issue key"),
...
server.registerTool("jira_get_comments", {
    ...
    description: "Get all comments on a JIRA issue. Returns comments with author, body (plain text), and timestamps. " +
...
// ---------------------------------------------------------------------------
server.registerTool("jira_assign_to_me", {
...
server.registerTool("tempo_log_time", {
    ...
    description: "Log worked time on a JIRA issue via Tempo. Time is specified in hours (supports decimals like 1.5).",
    inputSchema: {
        issueKey: z.string().describe("JIRA issue key"),
...
// ---------------------------------------------------------------------------
server.registerTool("tempo_get_today_worklogs", {
    title: "Get Today's Worklogs",
    description: "Get all time logged today via Tempo. Shows total hours and remaining to reach 8h. " +
...
// ---------------------------------------------------------------------------
server.registerTool("git_branch_info", {
    title: "Get Git Branch Info",
    description: "Get current branch name, detected JIRA issue key, commits on branch (vs base), " +
...
server.registerTool("git_create_branch", {
    ...
    description: "Create a new git branch from the base branch. Pulls latest changes first. " +
...
// ---------------------------------------------------------------------------
server.registerTool("detect_instance", {
    title: "Detect JIRA Instance",
    description: "Show which JIRA/Tempo instance is resolved for the given working directory. " +
...
server.registerTool("jira_search_issues", {
    ...
    description: "Search for JIRA issues using JQL or simple filters. " +
...
server.registerTool("jira_my_issues", {
    title: "Get My Issues (All Instances)",
    description: "Get all JIRA issues assigned to you across ALL configured JIRA instances (mmw, pares, lagoasoft). " +
    ...
    for (const resp of responses) {
        if ("error" in resp) {
            ...
            const filteredIssues = showResolved
                ? resp.result.issues
                : resp.result.issues.filter((issue) => !isResolvedStatus(issue.status));
            results[resp.name] = {
...
server.registerTool("jira_get_sprint", {
    ...
    description: "Get the current active sprint for a project board, including all issues and progress stats.",
    inputSchema: {
...
// Tool: jira_get_epic
// ---------------------------------------------------------------------------
server.registerTool("jira_get_epic", {
